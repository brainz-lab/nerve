...
# Nerve - API Gateway & Rate Limiting
...
BrainzLab.configure do |config|
  ...
  config.service = "nerve"
  config.environment = Rails.env

  # Ignore internal BrainzLab hosts to prevent infinite recursion
  config.http_ignore_hosts = %w[localhost 127.0.0.1 recall reflex pulse signal flux nerve]
end
...
Rails.application.config.after_initialize do
  ...
  Rails.logger.info "[Nerve] SDK initialized"
  Rails.logger.info "[Nerve] Recall logging: #{BrainzLab.configuration.recall_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Nerve] Reflex error tracking: #{BrainzLab.configuration.reflex_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Nerve] Pulse APM: #{BrainzLab.configuration.pulse_enabled ? 'enabled' : 'disabled'}"
end
